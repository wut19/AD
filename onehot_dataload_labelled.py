...
class OnehotDataset:
    ...
    def pairs_separate(data):
        reviews = [pair[1][:200] for pair in data]
        labels = [pair[0] for pair in data]
        reviews,labels = np.array(reviews),np.array(labels)
        # print(reviews.shape)
...
def make_datasets(data_path='./dataset/onehot_data.pkl'):
    ...
    l = min(l1,l2)
    train_data_true = true_onehot[:3*l1]  
    train_data_fake = fake_onehot[:3*l2]
    ...
    train_set_true = OnehotDataset(train_data_true)
    train_set_fake = OnehotDataset(train_data_fake)
    val_set = OnehotDataset(val_data)
    test_set = OnehotDataset(test_data)
    print("dataset is ready!")
    
    return train_set_true,train_set_fake,val_set,test_set
